...
using System.IO;
using GeneralToolkitLib.Compression.SevenZip.Compress.LZ;
using GeneralToolkitLib.Compression.SevenZip.Compress.RangeCoder;

namespace GeneralToolkitLib.Compression.SevenZip.Compress.LZMA
{
    public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
        ...
        public void SetCoderProperties(CoderPropID[] propIDs, object[] coderProperties)
        {
            for (uint i = 0; i < coderProperties.Length; i++)
            {
                object prop = coderProperties[i];
                switch (propIDs[i])
                {
        ...
        {
            return _isRepG0[state.Index].GetPrice0() + _isRep0Long[(state.Index << Base.kNumPosStatesBitsMax) + posState].GetPrice0();
        }
        ...
        private uint GetOptimum(uint position, out uint backRes)
        {
            ...
            {
                uint offs = 0;
                // ReSharper error LoopVariableIsNeverChangedInsideLoop
            ...
            while (true)
            {
                ...
                uint curAnd1Price = curPrice + _isMatch[(state.Index << Base.kNumPosStatesBitsMax) + posState].GetPrice0()
                ...
                for (uint repIndex = 0; repIndex < Base.kNumRepDistances; repIndex++)
                {
                    ...
                    if (lenTest < numAvailableBytesFull)
                    {
                        ...
                        if (lenTest2 >= 2)
                        {
                            ...
                            uint curAndLenCharPrice = repMatchPrice + GetRepPrice(repIndex, lenTest, state, posState) + _isMatch[(state2.Index << Base.kNumPosStatesBitsMax) + posStateNext].GetPrice0()
                                                      + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte((int) lenTest - 1 - 1))
                                                                       .GetPrice(true, _matchFinder.GetIndexByte((int) lenTest - 1 - (int) (reps[repIndex] + 1)), _matchFinder.GetIndexByte((int) lenTest - 1));
                            state2.UpdateChar();
                        ...
                        if (lenTest == _matchDistances[offs])
                        {
                            if (lenTest < numAvailableBytesFull)
                            {
                                ...
                                if (lenTest2 >= 2)
                                {
                                    ...
                                    state2.UpdateMatch();
                                    uint posStateNext = (position + lenTest) & _posStateMask;
                                    uint curAndLenCharPrice = curAndLenPrice + _isMatch[(state2.Index << Base.kNumPosStatesBitsMax) + posStateNext].GetPrice0()
                                                              + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte((int) lenTest - 1 - 1))
                                                                               .GetPrice(true, _matchFinder.GetIndexByte((int) lenTest - (int) (curBack + 1) - 1), _matchFinder.GetIndexByte((int) lenTest - 1));
                                    state2.UpdateChar();
                                    posStateNext = (position + lenTest + 1) & _posStateMask;
                                    uint nextMatchPrice = curAndLenCharPrice + _isMatch[(state2.Index << Base.kNumPosStatesBitsMax) + posStateNext].GetPrice1();
                                    uint nextRepMatchPrice = nextMatchPrice + _isRep[state2.Index].GetPrice1();
        ...
        public class CodeProgress
        {
            ...
            public long ProcessedInSize;
            public long ProcessedOutSize;
        }
